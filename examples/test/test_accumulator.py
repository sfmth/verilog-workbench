...
async def test_shift_add_mult(dut):
    ...
    dut.oen.value = 1
    accumulator_expected = 0

    for i in range(1000):
        spk_in = random.randint(0, 3)
        w_read = random.randint(0, 3)

        synapse = (w_read & spk_in)
        synapse_count = ((synapse >> 0) & 1) + ((synapse >> 1) & 1)
        accumulator_expected = min(accumulator_expected + synapse_count, 255)

        dut.spk_in.value = spk_in
        dut.w_read.value = w_read

        await RisingEdge(dut.clk)
        await FallingEdge(dut.clk)

        assert bin_to_int(dut.accumulated_potential.value) == accumulator_expected, \
            f"Step {i}: expected {accumulator_expected}, got {bin_to_int(dut.accumulated_potential.value)}"
